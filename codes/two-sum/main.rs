...
impl TwoSum {
    fn brute_force(&self, nums: Vec<i32>, target: i32) -> Vec<i32> {
    ...
    fn two_pass_hash_table(&self, nums: Vec<i32>, target: i32) -> Vec<i32> {
    ...
    fn one_pass_hash_table(&self, nums: Vec<i32>, target: i32) -> Vec<i32> {
        let mut lookup = HashMap::new();
        for (i, num) in nums.iter().enumerate() {
            let diff = target - num;
            if let Some(&j) = lookup.get(&diff) {
...
fn main() {
    ...
    let approaches: Vec<fn(&TwoSum, Vec<i32>, i32) -> Vec<i32>> = vec![
        TwoSum::brute_force,
